      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            ...
            className="rounded-lg border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-gray-600 transition-colors hover:bg-indigo-50 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
          >
          ...
          <button
            ...
            className="rounded-lg p-1.5 text-gray-300 transition-colors hover:bg-red-50 hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400"
          >
...
function triggerPDFExport(questions: QueuedQuestion[]) {
  ...
  const drawHeader = () => {
    ...
    doc.text("QUESTION PAPER", marginX, 38);

    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(71, 85, 105);
    doc.text(`Subject: ${subject}`, marginX, 58);
    doc.text(`Date: ${dateStr}`, pageWidth - marginX - 110, 58);
    doc.text(`Total Questions: ${questions.length}    Total Marks: ${totalMarks}`, marginX, 74);
  ...
  const drawFooter = () => {
    ...
    if (typeof doc.textWithLink === "function") {
      doc.textWithLink("Generated by AI Hub", marginX, footerY, { url: footerUrl });
    } else {
      doc.text("Generated by AI Hub", marginX, footerY);
    }
  };
  ...
  const addQuestionBlock = (question: QueuedQuestion, index: number, startY: number) => {
    ...
    if (question.sourceLabel) {
      doc.setFontSize(8);
      doc.setTextColor(100, 116, 139);
      doc.text(`Source: ${question.sourceLabel}`, marginX + 18, bodyY + textHeight + 10);
    }

    const answerY = bodyY + textHeight + 30;
    doc.setDrawColor(226, 232, 240);
